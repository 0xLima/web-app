...
import LtvAPI from '../../../scripts/ZapScore';
export default {
    data() {
        return {
    ...
    methods: {
        generateImages: function () {
         ...
         getLtv: async function () {
            if (this.userAddress) {
                this.userLtv = await LtvAPI.getZapScore(this.userAddress)
            }
        },
        getProfile: async function () {
            ...
            this.axios.get(`https://zapsterfinance.onrender.com/users/${this.userAddress}`).then(response => {
